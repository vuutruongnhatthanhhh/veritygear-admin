...
import { upsertFeatureItem } from "../actions";
...
export function FeatureForm({ item }: { item: FeatureItem | null }) {
  const [error, action, pending] = useActionState(upsertFeatureItem, null);
  const prevPending = useRef(false);
  const router = useRouter();

  useEffect(() => {
    if (prevPending.current && !pending && !error) {
      announceToast(item ? "Đã lưu thay đổi" : "Đã thêm tính năng");
      router.push("/trang-chu/tinh-nang");
    }
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        >
          {pending ? "Đang lưu..." : item ? "Lưu thay đổi" : "Thêm tính năng"}
        </button>
        <Link href="/trang-chu/tinh-nang" className="text-sm text-zinc-500 transition hover:text-zinc-900">
          Hủy
        </Link>
      </div>
    </form>
