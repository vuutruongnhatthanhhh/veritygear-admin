...
function removeImageIfOwned(admin: ReturnType<typeof createAdminClient>, url: string | null) {
  if (!url) return;
  const path = url.split("/contact-images/")[1];
  if (path) admin.storage.from("contact-images").remove([path]);
}

export async function upsertContactHero(_prev: string | null, formData: FormData): Promise<string | null> {
  ...
  if (deleteImage || selectedUrl) {
    const { data: current } = await admin.from("contact_hero").select("image_url").eq("id", 1).single();
    if (current?.image_url && current.image_url !== selectedUrl) {
      removeImageIfOwned(admin, current.image_url);
    }
  }

  const { error } = await admin.from("contact_hero").update(payload).eq("id", 1);
